...
+(NSString*)removeEmoji:(NSString*)string {
    ...
    while(i < [string length]) {
        ...
        // マッチしたかのフラグを文字ごとにリセット
        matched = NO;
        
        // コードにマッチするものがあるかチェック
        for(int j = 0; j < arySize; j++) {
            if(code == ary[j][0]) {
                // 1文字目がマッチしたとき、2文字目をマッチする必要がなければ削除対象、2文字があればそちらもチェック
                if(ary[j][1]) {
                    if(i + 1 < [string length]) {
                        unichar code2 = [string characterAtIndex:i + 1];
                        if(code2 == ary[j][1]) {
                            matched = YES;
                            i++; // 2文字でマッチしたら、カウンターを1つ余分に進めておく
                            break;
                        }
                    }
                } else {
